...
async def verify_claim(req: VerificationRequest):
    ...
    if not image_path.startswith(os.path.abspath("/secure/local/volume/")):
        logger.error(f"Unauthorized access attempt to path: {image_path}")
        raise HTTPException(status_code=403, detail="Unauthorized access to the file path.")
